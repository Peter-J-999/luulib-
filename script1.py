...
def date_if(prefer_sit, driver):
    ...
    if times_roll and len(times_roll) > 0:
        ...
        if all(time in full_day_times for time in get_time_range(full_day_times, "10:00", "15:00")):
            day_type = 3
...
def choose_it(driver, sit_avilable, idx, reading_room, day_type, max_attempts=500):
    ...
    bool: 预约是否成功
    """
    dir_time = {3: [['10:00', '15:00'], ['10:00', '15:00']],
    ...
    while attempt < max_attempts:
        ...
        if attempt >= 2:
            try:
                close_btn = driver.find_element(By.CSS_SELECTOR, "p.close-icon")
                close_btn.click()
                time.sleep(0.5)
            except:
                pass
            try:
                ...
                driver.execute_script("arguments[0].click();", sit_elem)
            except Exception as e:
                print("点击座位失败:", e)
        try:
            # 检查位置是否还可约全天
            ...
            if len(second_roll_sections) >= 2:
                ...
                if not end_time_found:
                    print(f"结束时间{end_time}已被抢，执行随机座位程序")
                    return False
            ...
            if reserve_box:
                ...
                if times_roll:
                    ...
                    if all(time in time_texts for time in
                           get_time_range(full_day_times, "10:00", "15:00")):
                        print(f"找到全天可预约座位: {i}")
                        found_full_day = True
                        day_type = 3
...
def main():
    ...
    sit_avilable, day_type = None, None
    users = {"自定义": [account_password4, "四楼北自习室", "2"]}
    user = "自定义"
